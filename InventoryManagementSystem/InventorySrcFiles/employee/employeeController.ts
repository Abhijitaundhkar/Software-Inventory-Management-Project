...
export class employeeController{
           ...
           public addNewEmployee(){
              ...
              handler: async function (request:any, h: Hapi.ResponseToolkit) {
               try{
                 ...
                 const getTableData=await employeeTableData.findAll()
               let isTemp=true 
               //find the data is present or not
                for(let i of getTableData){
               if(i.email===request.query.email)
               {
                isTemp=false
                return h.response('already email id registered')
             
                }
                 else if(i.phoneNo===request.query.phoneNo)
                {
                  isTemp=false
      ...
      // get all employee details
        ...
        public updateEmployeeDetails() {
          return {
            method: "PUT",
            path: "/updateEmployeeDetails/{employeeId}",
            handler: async (request: any, h: Hapi.ResponseToolkit) => {
              try {
                const dataBase = await request.getDb("InventoryManagementDatabase");
                const employeeTableData = dataBase.models.employee;
              ...
              if (result) {
                // result.employeeId=request.payload.employeeId,
        ...
        deleteEmployeeDetails() {
          return {
            method: "DELETE",
            path: "/deleteEmployeeDetails/{employeeId}",
            handler: async (request: any, h: Hapi.ResponseToolkit) => {
              // try {
                const dataBase = await request.getDb("InventoryManagementDatabase");
                const employeeTableData = dataBase.models.employee;
          ...
          return{
             ...
             handler: async function (request: any, h: Hapi.ResponseToolkit) {
              try {
               ...
               //check securityQuestion,securityAnswer is correct or not
               const getOutput = await employeeTableData.findOne({ 
                where: {email:email } 
             });
        ...
        //          const employeeTableData = dataBase.models.employee;
        //          //  get all data from table
        //          const getTableData=await employeeTableData.findOrCreate({
